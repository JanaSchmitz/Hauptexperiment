...
public class TRASH extends Statement {
    ...
    public void generateTrash(int lineCount) {
        Random random = new Random();
        boolean r = random.nextBoolean();
        if (r == true) {
            GenerateTrashSwitch(lineCount);
    ...
    public void GenerateTrashSwitch(int lineCount) {
        String trashSwitch;
        String name = words.getFreeName();
        words.usedidentifierNames.add(name);
        trashSwitch = "switch(" + name.toLowerCase() + "){";
        setTrashSwitch(trashSwitch);
        generateTrashCase(lineCount);

    }

    public String[] generateTrashCase(int lineCount) {
        int random;
        if(param.getMaxLines() - lineCount < 5){
            //System.out.println("nur 2");
           random = 1;
           if(param.getMaxLines() - lineCount < 8){
               random = 2;
           }
        }else {
            random = ThreadLocalRandom.current().nextInt(2, 4);
        }
    ...
    public String createIf() {
        ...
        trashif = "if(" + names[1].toLowerCase() + " == " + names[1] + "." + names[2].toLowerCase() + "){";
        words.usedidentifierNames.add(names[1]);
        return trashif;
    }
