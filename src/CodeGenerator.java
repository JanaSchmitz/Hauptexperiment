...
public class CodeGenerator {
    ...
    public String generateCode(){
        ...
        for(int i = 0; i < shuffeldEnums.length; i++) {
            ...
            EnumNames.add(shuffeldEnums[i].getName());
        }
        code += "\n";
        code += "   " + "public void ChoooseStatements(" ;
        for(int i = 0; i < words.usedidentifierNames.size(); i++) {
            code += words.usedidentifierNames.get(i) + " " + words.usedidentifierNames.get(i).toLowerCase();
            if(words.usedidentifierNames.size()-1 != i){
                code += ", ";
            }
        }
        code+=  ") {" + "\n";
        ...
        for(int i = 0; i < EnumNames.size(); i++){
            code += EnumNames.get(i) +  " " + EnumNames.get(i).toLowerCase() + ";" + " ";
        }
        code += "\n";
        code += "\n";
/*
